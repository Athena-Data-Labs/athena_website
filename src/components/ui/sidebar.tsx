...
>(({ className, showIcon = false, ...props }, ref) => {
  const widths = ["w-1/2", "w-[56%]", "w-[62%]", "w-[68%]", "w-[74%]", "w-[80%]", "w-[86%]", "w-[90%]"];
  const widthClass = React.useMemo(() => widths[Math.floor(Math.random() * widths.length)] ?? "w-1/2", []);
  ...
  return (
    ...
    >
      ...
      <Skeleton
        className={cn("h-4 max-w-full flex-1", widthClass)}
